...
<?php
	...
	$sql_update="SELECT `point_update` FROM `match_result` WHERE `match_id` LIKE '$match_id'";
	
	$query=mysqli_query($connection,$sql_update);
	$result=mysqli_fetch_array($query);
	$updated=$result['point_update'];

	if ($updated==1){

		echo "Results already updated";
	...
	$sql="SELECT u.`reg_id`,u.`user_id`, p.`team1_score` AS predict1,p.`team2_score` as predict2, p.`home_scorers` AS predicted_Scorers_h, p.`away_scorers` AS predicted_Scorers_a FROM `prediction` AS p JOIN `user` AS u WHERE u.`reg_id` LIKE p.`user_id`  && p.`match_id` LIKE '$match_id'";
	...
	while($array=mysqli_fetch_array($query)){
		...
		echo "<br><br><br>";
		$insertPointBreakdown=$insertPointBreakdown."('".$match_id."','".$reg_id."','".$points."'),";	
	...
	$insertPointBreakdown=substr($insertPointBreakdown,0,-1);
	
	$insertPointQuery="INSERT INTO `points_breakdown`  (`match_id`,`user_id`, `point`) VALUES ".$insertPointBreakdown;
	mysqli_query($connection,$insertPointQuery);
	
	$insertQuery="UPDATE `user_details` AS U SET U.`points` = (SELECT SUM(point) FROM `points_breakdown` AS P WHERE U.`user_id` LIKE P.`user_id`)";
	mysqli_query($connection,$insertQuery);
	$update_points_query="UPDATE `match_result` SET point_update = 1 WHERE `match_id` LIKE '$match_id'";
	mysqli_query($connection,$update_points_query);
